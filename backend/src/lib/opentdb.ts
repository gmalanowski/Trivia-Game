 ...
 */
export async function fetchQuestions(options: FetchQuestionsOptions): Promise<OpenTDBQuestion[]> {
	...
	if (!response.ok) {
		throw new Error(`Failed to fetch from OpenTDB API: ${response.status} ${response.statusText}`);
	}
	...
	switch (data.response_code) {
		case 0: // Success
			return data.results;
		case 1:
			throw new OpenTDBError(1, "No Results: Could not return results. The API doesn't have enough questions for your query. (ex. Asking for 50 Questions in a Category that only has 20.)");
		case 2:
		...
		case 4:
			throw new OpenTDBError(4, "Token Empty: Session Token has returned all possible questions for the specified query.");
		case 5:
			throw new OpenTDBError(5, "Rate Limit: Too many requests have occurred. Each IP can only access the API once every 5 seconds.");
		default:
			throw new OpenTDBError(data.response_code, `Unknown OpenTDB response code: ${data.response_code}`);
	}
}
